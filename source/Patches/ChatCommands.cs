        ...
        public static class PrivateJaileeChat
        {
            public static bool Prefix(ChatController __instance, [HarmonyArgument(0)] ref PlayerControl sourcePlayer, ref string chatText)
            {
                if (sourcePlayer == PlayerControl.LocalPlayer)
                {
                    ...
                    else if (chatText.ToLower().StartsWith("/shift") || chatText.ToLower().StartsWith("/ shift"))
                    {
                        AddRoleMessage(RoleEnum.Shifter);
                        return false;
                    }
                    ...
                    else if (chatText.ToLower().StartsWith("/camou") || chatText.ToLower().StartsWith("/ camou"))
                    {
                        AddRoleMessage(RoleEnum.Camouflager);
                        return false;
                    }
            ...
            public static void AddRoleMessage(RoleEnum role)
            {
                ...
                if (role == RoleEnum.TimeLord) DestroyableSingleton<HudManager>.Instance.Chat.AddChat(
                    PlayerControl.LocalPlayer, "The Time Lord is can rewind the time.");
                if (role == RoleEnum.Mayor) DestroyableSingleton<HudManager>.Instance.Chat.AddChat(
                    PlayerControl.LocalPlayer, "The Mayor which can abstain its vote for the next round.");
                if (role == RoleEnum.President) DestroyableSingleton<HudManager>.Instance.Chat.AddChat(
                    PlayerControl.LocalPlayer, "The President is a crewmate with 3 votes and their role is revealed to everyone.");
                ...
                if (role == RoleEnum.Camouflager) DestroyableSingleton<HudManager>.Instance.Chat.AddChat(
                    PlayerControl.LocalPlayer, "The Camouflager can turn everyone gray.");
